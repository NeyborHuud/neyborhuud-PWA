...
function statusBadgeClass(status: KidnappingTrackingSession['status']): string {
  switch (status) {
    case 'active':
      return 'bg-red-900/60 text-brand-red border border-red-600';
    case 'lost_signal':
      return 'bg-brand-red900/60 text-brand-red300 border border-orange-600';
    case 'ended':
      return 'bg-brand-black text-[var(--neu-text-muted)] border border-black/[0.08]';
    default:
      return 'bg-brand-black text-[var(--neu-text-muted)] border border-black/[0.08]';
  }
}
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="flex max-w-7xl mx-auto">
        ...
        {/* ── Main Content ── */}
        <main className="flex-1 min-w-0 border-x border-black/[0.08] px-4 py-6 space-y-6">
          {/* Header */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link
                href="/safety"
                className="text-[var(--neu-text-muted)] hover:text-white transition-colors text-sm"
              >
                ← Safety
              </Link>
              <h1 className="text-xl font-bold text-brand-red">🚨 Kidnapping Tracker</h1>
            </div>
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded-full border ${
                wsStatus === 'connected'
                  ? 'bg-green-900/40 text-primary border-green-700'
                  : wsStatus === 'connecting'
                  ? 'bg-primary900/40 text-primary400 border-yellow-700'
                  : 'bg-brand-black text-[var(--neu-text-muted)] border-black/[0.08]'
              }`}
            >
          ...
          {/* WS alert banner */}
          {wsAlert && (
            <div className="bg-brand-red900/40 border border-orange-600 rounded-lg px-4 py-3 text-brand-red300 text-sm flex items-start justify-between gap-2">
              <span>{wsAlert}</span>
              <button
                onClick={() => setWsAlert(null)}
                className="text-brand-red hover:text-brand-red shrink-0"
              >
                ✕
              </button>
            </div>
          ...
          {loadingSession ? (
            <div className="flex items-center justify-center py-12 text-[var(--neu-text-muted)]">
              Loading session…
            </div>
          ) : (
            <>
              {/* ── Session Status Card ── */}
              {activeSession ? (
                <div className="bg-brand-black border border-red-800 rounded-xl p-5 space-y-4">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[var(--neu-text-muted)] text-sm">
                        Session <span className="font-mono text-[var(--neu-text-muted)]">{activeSession._id.slice(-8)}</span>
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      {/* Online / queue indicator */}
                      {!tracking.isOnline && (
                        <span className="text-xs bg-brand-red900/40 text-brand-red border border-orange-700 px-2 py-0.5 rounded-full">
                      ...
                      {tracking.isOnline && tracking.queuedCount > 0 && (
                        <button
                          ...
                          className="text-xs bg-blue-900/40 text-brand-blue border border-blue-700 px-2 py-0.5 rounded-full hover:bg-blue-900/60"
                        >
                      ...
                      <button
                        onClick={handleStop}
                        className="text-sm bg-red-900/50 hover:bg-red-900/80 text-brand-red border border-red-700 px-4 py-1.5 rounded-lg transition-colors"
                      >
                        Stop Tracking
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                    <div className="bg-brand-black rounded-lg p-3">
                      <div className="text-[var(--neu-text-muted)] mb-0.5">Type</div>
                      <div className="text-white capitalize">{activeSession.emergencyType.replace('_', ' ')}</div>
                    </div>
                    <div className="bg-brand-black rounded-lg p-3">
                      <div className="text-[var(--neu-text-muted)] mb-0.5">Interval</div>
                      <div className="text-white">{activeSession.intervalSeconds}s</div>
                    </div>
                    <div className="bg-brand-black rounded-lg p-3">
                      <div className="text-[var(--neu-text-muted)] mb-0.5">Activated by</div>
                      <div className="text-white capitalize">{activeSession.activatedBy}</div>
                    </div>
                    <div className="bg-brand-black rounded-lg p-3">
                      <div className="text-[var(--neu-text-muted)] mb-0.5">Missed pings</div>
                      <div className={activeSession.missedPings > 0 ? 'text-brand-red' : 'text-white'}>
                  ...
                  <div
                    className={`text-xs px-3 py-2 rounded-lg ${
                      activeSession.authorityNotified
                        ? 'bg-green-900/30 border border-green-700 text-primary'
                        : 'bg-brand-black border border-black/[0.08] text-[var(--neu-text-muted)]'
                    }`}
                  >
                    {activeSession.authorityNotified ? (
                      <>
                  ...
                  {/* Latest location */}
                  {tracking.latestLocation && (
                    <div className="bg-brand-black border border-black/[0.08] rounded-lg p-3 text-xs">
                      <div className="text-[var(--neu-text-muted)] mb-1">Latest location</div>
                      <div className="text-white font-mono">
                        {tracking.latestLocation.location.lat.toFixed(6)},{' '}
                        {tracking.latestLocation.location.lng.toFixed(6)}
                      </div>
                      <div className="flex gap-4 mt-1 text-[var(--neu-text-muted)]">
                  ...
                  {summary ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                      <div className="bg-brand-black rounded-lg p-3">
                        <div className="text-[var(--neu-text-muted)] mb-0.5">Distance</div>
                        <div className="text-white">{formatDistance(summary.totalDistanceMeters)}</div>
                      </div>
                      <div className="bg-brand-black rounded-lg p-3">
                        <div className="text-[var(--neu-text-muted)] mb-0.5">Points</div>
                        <div className="text-white">{summary.totalPoints}</div>
                      </div>
                      <div className="bg-brand-black rounded-lg p-3">
                        <div className="text-[var(--neu-text-muted)] mb-0.5">Duration</div>
                        <div className="text-white">{formatDuration(summary.durationSeconds)}</div>
                      </div>
                      <div className="bg-brand-black rounded-lg p-3">
                        <div className="text-[var(--neu-text-muted)] mb-0.5">Avg speed</div>
                        <div className="text-white">{summary.avgSpeedKmh.toFixed(1)} km/h</div>
                      </div>
                      <div className="bg-brand-black rounded-lg p-3">
                        <div className="text-[var(--neu-text-muted)] mb-0.5">Max speed</div>
                        <div className="text-white">{summary.maxSpeedKmh.toFixed(1)} km/h</div>
                      </div>
                      <div className="bg-brand-black rounded-lg p-3">
                        <div className="text-[var(--neu-text-muted)] mb-0.5">Stationary</div>
                    ...
                    <button
                      ...
                      className="text-xs text-brand-blue hover:text-brand-blue underline"
                    >
                      Load summary
                    </button>
                  )}
                ...
                <div className="bg-brand-black border border-black/[0.08] rounded-xl p-5 space-y-4">
                  <h2 className="text-lg font-semibold text-brand-red">Start Emergency Tracking</h2>
                  ...
                  {startError && (
                    <div className="bg-red-900/40 border border-red-700 rounded-lg px-4 py-2 text-brand-red text-sm">
                      {startError}
                    </div>
                  )}

                  <div className="space-y-3">
                    <label className="block text-sm text-[var(--neu-text-muted)]">
                      Emergency type
                      <select
                        value={emergencyType}
                        onChange={(e) => setEmergencyType(e.target.value as KidnappingEmergencyType)}
                        className="mt-1 block w-full bg-brand-black border border-black/[0.08] text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red"
                      >
                    ...
                    <label className="block text-sm text-[var(--neu-text-muted)]">
                      ...
                      <input
                        ...
                        className="mt-1 block w-full bg-brand-black border border-black/[0.08] text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red"
                      />
                      <span className="text-[var(--neu-text-muted)] text-xs">Lower = more precise, higher battery use</span>
                    </label>
                  </div>
              ...
              {locationHistory.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-semibold text-[var(--neu-text-muted)]">
                  ...
                  <div className="space-y-1 max-h-96 overflow-y-auto pr-1">
                    {locationHistory.map((point, idx) => (
                      <div
                        key={point._id || idx}
                        className="bg-brand-black border border-black/[0.08] rounded-lg px-3 py-2 flex items-start justify-between gap-2 text-xs"
                      >
                        <div>
                          <span className="font-mono text-[var(--neu-text-muted)]">
                            {point.location.lat.toFixed(6)}, {point.location.lng.toFixed(6)}
                          </span>
                          {point.location.address && (
                            <span className="text-[var(--neu-text-muted)] ml-2 truncate max-w-[180px]">
                              {point.location.address}
                            </span>
                          )}
                          <div className="flex gap-3 mt-0.5 text-[var(--neu-text-muted)]">
                        ...
                        <span className="text-[var(--neu-text-secondary)] shrink-0">{formatTimestamp(point.timestamp)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Error */}
              {tracking.error && (
                <div className="bg-red-900/30 border border-red-700 rounded-lg px-4 py-3 text-brand-red text-sm">
                  ⚠️ {tracking.error}
                </div>
              )}
            </>
