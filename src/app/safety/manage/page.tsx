...
function SafetyPageInner() {
  ...
  useEffect(() => {
    if (sos.activeSos?._id) {
      loadGuardianActivity(sos.activeSos._id);
    } else {
      setGuardianActivity([]);
    }
  }, [sos.activeSos?._id, loadGuardianActivity]);
  ...
  useEffect(() => {
    ...
    socket.on('safety:sos_alert', () => {
      // Hook owns the canonical SOS state; this is just a refresh trigger for guardian activity.
      if (sos.activeSos?._id) loadGuardianActivity(sos.activeSos._id);
    });
  ...
  const onCancelSos = async () => {
    await sos.cancelSos('False alarm');
  };
            ...
            <section className="neu-card-sm rounded-2xl p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  ...
                  disabled={sos.loading || sos.phase === 'pending'}
                  className="px-5 py-2.5 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors disabled:opacity-50"
                >
                  {sos.phase === 'pending' ? 'Arming…' : 'Trigger SOS'}
                </button>

                {sos.activeSos && sos.phase === 'active' && (
                  <>
                    <button
              ...
              <div className="mt-3 text-xs" style={{ color: 'var(--neu-text-muted)' }}>
                {sos.activeSos
                  ? sos.phase === 'pending'
                    ? `Pending · ${sos.secondsRemaining}s remaining (${sos.activeSos.visibilityMode} mode)`
                    : `Active SOS: ${sos.activeSos.visibilityMode} mode · escalation level ${sos.activeSos.escalationLevel}`
                  : 'No active SOS event.'}
              </div>
              ...
              {sos.lastSummary && (
                <div className="mt-3 neu-socket rounded-xl p-3 border border-green-700/40">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs" style={{ color: 'var(--neu-text-muted)' }}>
                    ...
                    <div>Acknowledged: {sos.lastSummary.guardians.acknowledgedCount}</div>
                    <div>Tracking pings: {sos.lastSummary.tracking.pingsLogged}</div>
                    {sos.lastSummary.guardians.fastestResponseMs !== null && (
                      <div className="col-span-2">
                        Fastest response: {Math.round((sos.lastSummary.guardians.fastestResponseMs ?? 0) / 1000)}s
                      </div>
                    )}
                    {sos.lastSummary.agencyDispatch.agency && (
                      <div className="col-span-2">
                        Agency: {sos.lastSummary.agencyDispatch.agency} ({sos.lastSummary.agencyDispatch.status})
                      </div>
                    )}
                  </div>
                </div>
