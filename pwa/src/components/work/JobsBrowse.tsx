...
/**
 * JobsBrowse — the jobs filters + listing grid, without the page chrome.
 * Reused by both /jobs and the unified /work hub (Hiring tab).
 */
...
export function JobsToolbar({
  ...
  return (
    <div className="space-y-3">
      ...
      <JobFilters filters={filters} onChange={onChange} />
    </div>
  );
}

interface JobsBrowseProps {
  /** Controlled filter state (so the hub/page can render the toolbar in its header). */
  filters?: JobsFilterState;
}

export function JobsBrowse({ filters: controlledFilters }: JobsBrowseProps) {
  const [internalFilters] = useState<JobsFilterState>(DEFAULT_FILTERS);
