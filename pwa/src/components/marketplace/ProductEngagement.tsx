...
export function ProductEngagement({
  ...
  return (
    <div className="flex items-center gap-4 border-t border-black/[0.08] pt-4">
      {/* Like Button */}
      <button
        onClick={handleLikeClick}
        disabled={isLikePending || !currentUserId}
        className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-brand-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed group"
        aria-label={isLiked ? "Unlike this product" : "Like this product"}
      >
        <svg
          className={`w-6 h-6 transition-all ${
            isLiked
              ? "text-brand-red fill-current scale-110"
              : "text-[var(--neu-text-muted)] group-hover:text-brand-red group-hover:scale-110"
          }`}
        ...
        <span className={isLiked ? "text-brand-red font-semibold" : "text-[var(--neu-text-muted)]"}>
      ...
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-brand-black transition-colors group"
        aria-label="View comments"
      >
        <svg
          className="w-6 h-6 text-[var(--neu-text-muted)] group-hover:text-brand-blue transition-colors"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
