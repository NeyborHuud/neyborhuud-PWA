// src/lib/geolocation.ts
import { getGeolocation } from './nativeGeolocation';

export interface LocationCoords {
    lat: number;
    lng: number;
    accuracy?: number;
}

/**
 * Get user's current GPS location
 * Prompts browser for location permission
 * Tries multiple times to get better accuracy
 */
export const getCurrentLocation = (): Promise<LocationCoords | null> => {
    return new Promise((resolve) => {
        const geo = getGeolocation();
        if (!geo) {
            console.warn('Geolocation not supported');
            resolve(null);
            return;
        }

        let bestLocation: LocationCoords | null = null;
        let attempts = 0;
        const maxAttempts = 3;
        let settled = false;

        const finish = (loc: LocationCoords | null) => {
            if (settled) return;
            settled = true;
            resolve(loc);
        };

        const tryGetLocation = () => {
            attempts++;
            // High accuracy on the first pass; relax to network/WiFi positioning on
            // retries so devices without GPS (desktops, laptops) still resolve a
            // location instead of timing out repeatedly.
            const highAccuracy = attempts === 1;
            console.log(`📍 Getting location (attempt ${attempts}/${maxAttempts}, highAccuracy=${highAccuracy})...`);

            geo.getCurrentPosition(
                (position) => {
                    const coords = {
                        lat: position.coords.latitude,
                        lng: position.coords.longitude,
                        accuracy: position.coords.accuracy
                    };

                    console.log(`📍 Location received: accuracy ${coords.accuracy}m`);

                    // Keep the most accurate result
                    if (!bestLocation || coords.accuracy < (bestLocation.accuracy || Infinity)) {
                        bestLocation = coords;
                    }

                    // If accuracy is good enough (< 100m) or we've tried enough times, return
                    if (coords.accuracy < 100 || attempts >= maxAttempts) {
                        if (coords.accuracy > 1000) {
                            console.warn(`⚠️ Location accuracy is poor: ${coords.accuracy}m. This may affect address resolution.`);
                        }
                        finish(bestLocation);
                    } else {
                        // Try again for better accuracy
                        setTimeout(tryGetLocation, 500);
                    }
                },
                (error) => {
                    // Timeout (code 3) or position-unavailable (code 2) on a high-accuracy
                    // pass: retry with relaxed settings before giving up.
                    const retryable = (error.code === error.TIMEOUT || error.code === error.POSITION_UNAVAILABLE);
                    if (retryable && attempts < maxAttempts) {
                        console.warn(`Location attempt ${attempts} failed (${error.message}); retrying with relaxed accuracy…`);
                        setTimeout(tryGetLocation, 300);
                        return;
                    }
                    console.warn('Location permission denied or unavailable:', error);
                    // If we have any location, use it; otherwise return null
                    finish(bestLocation);
                },
                {
                    enableHighAccuracy: highAccuracy,
                    // Allow more time, and accept a recently cached fix on retries.
                    timeout: highAccuracy ? 8000 : 12000,
                    maximumAge: highAccuracy ? 0 : 60000,
                }
            );
        };

        tryGetLocation();
    });
};
