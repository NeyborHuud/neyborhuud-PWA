...
function CommunityEmergencyInner({
  ...
  return (
    <>
      ...
      {!user ? (
        <div className="mod-card mb-4 flex items-center gap-3 rounded-2xl p-4">
          ...
          <p className="text-sm" style={{ color: 'var(--neu-text-muted)' }}>
            ...
            to post community emergency alerts and react to reports.
          </p>
        </div>
      ) : null}
      ...
      {!loading && !error && posts.length === 0 && (
        <BrowseEmptyState
          ...
          description="All clear in your neighbourhood. Post an alert if there is an active emergency."
          action={
            user ? (
              <LocalHuudHubPrimaryAction label="Post alert" onClick={() => setShowCreate(true)} />
            ) : undefined
          }
        />
      )}
...
function CommunityEmergencyPageContent() {
  ...
  return (
    <AppBrowseLayout
      ...
      header={
        <LocalHuudHubHeader
          hubId="community-emergency"
          toolbar={
            user ? (
              <div className="flex justify-end">
                <LocalHuudHubPrimaryAction
                  label={showCreate ? 'Cancel' : 'Post alert'}
                  icon={showCreate ? 'close' : 'add_alert'}
    ...
    >
      <CommunityEmergencyInner showCreate={showCreate} setShowCreate={setShowCreate} />
    </AppBrowseLayout>
  );
}
